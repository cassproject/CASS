...
function contactSearch() {
    ...
    var append = "";
    var visibleContacts = $("#contactsList").find(".contact:visible");
    for (var i = 0; i < visibleContacts.length; i++) {
        append += " OR (\"" + visibleContacts.find("#identity").attr("title") + "\")";
    }

    if ($("#contactSearch").val() != null && $("#contactSearch").val() != "") {
        $(".cass-competency").hide();
        $(".cass-framework").hide();
        $(".assertion").hide();
        anySearchContact("(" + $("#contactSearch").val() + ")" + append, function (competency) {
            $("[url='" + competency.shortId() + "']").show();
        }, function (assertion) {
            $("[url='" + assertion.shortId() + "']").show();
            var a = new EcAssertion();
            a.copyFrom(assertion);
            var subject = a.getSubject();
            if (subject != null)
                $(".contact [title=\"" + subject.toPem() + "\"]").parent().show();
        }, function (framework) {
            $("[url=\"" + framework.shortId() + "\"]").show();
        }, 1);
    } else {
        $(".cass-competency").show();
        $("#contactsList").find(".contact").show();
        $(".cass-framework").show();
        $(".assertion").show();
    }
}
